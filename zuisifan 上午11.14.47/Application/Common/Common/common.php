...
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0){
    ...
    if($update_user_money > 99999999.99 || $update_user_money < -99999999.99 || $update_pay_points < 0 || $update_pay_points > 4294967295){
        return array('is_success'=> false,'message'=>'超出账户数据范围！');
    }
    ...
    if( D('users')->execute($sql)){
        ...
        return array('is_success'=> true,'message'=>'');
    }else{
        return array('is_success'=> false,'message'=>'添加失败！');
    }
}
...
function calculate_price($user_id=0, $order_goods, $shipping_price=0, $pay_points=0, $user_money=0, $coupon_id=0, $couponCode='' ,$province=0,$city=0,$district=0)
{    
    ...
    $order_prom_amount = 0;
       ...
       $order_amount = ($goods_price * 100 + $shipping_price * 100 - $coupon_price * 100)/100; // 应付金额 = 商品价格 + 物流费 - 优惠券
