...
class UserController extends BaseController {
    ...
    public function _initialize() {      
        ...
        if(session('?user'))
        {
        	...
        	$nologin = array(
        			...
        			'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'show_qr_code',
        	);
    ...
    public function reg(){
        ...
        $lead_token = I('token', 0, 'trim');
        $lead_id = 0;
        if($lead_token){
            $lead_id = M('users')->where(array('token'=>$lead_token))->getField('user_id');
        ...
        if(IS_POST){
            ...
            $code = I('post.code','');
            $first_lead_id = I('post.first_lead_id', 0);
            //是否开启注册短信验证码机制
            ...
            $data = $logic->reg($username,$password,$password2, $first_lead_id);
            if($data['status'] != 1)
                $this->error($data['msg']);
    ...
    public function order_list(){
        ...
        $order_logic->check_order_outtime();
    ...
    public function order_detail(){
        ...
        if($order_info['shipping_status'] == 1){
        	$express = selectExpress($id);
            if($express['is_success']>0){
                $express = object_array($express['data']);
            }else{
                $express = '暂无物流信息！';
            }
    ...
    public function info(){
        ...
        if(IS_POST){
            I('post.nickname') ? $post['nickname'] = I('post.nickname','','trim') : false; //昵称			
    ...
    */
    ...
    public function send_sms_reg_code(){
        ...
        //改为XX短信接口发送
        $send = $userLogic->sms_liuniu_log($mobile,$code,$this->session_id);
        if($send['status'] != 1)
            exit(json_encode(array('status'=>-1,'msg'=>$send['msg'])));
        exit(json_encode(array('status'=>1,'msg'=>'验证码已发送，请注意查收')));
    }
    ...
    public function send_validate_code(){
    	...
    	$send = I('send');
    	$logic = new UsersLogic();
    	$res = $logic->send_validate_code_liuniu($send, $type);
        $this->ajaxReturn($res);
    }
    ...
    public function return_goods()
    {
        ...
        //判断订单是否已经分成
        $rebate_status = M('rebate_log')->field('status')->where(array('order_id'=>$order_id))->find();
        if($rebate_status && $rebate_status['status'] == 3){
            $this->success('该订单已分成，不可退款!');
            exit;
        }
        ...
        if(IS_POST)
        {
            ...
            $imgs_str = implode(',', I('imgs'));
            $data['imgs'] = $imgs_str; // 用户拍照的相片
            ...
            if($return_id){
                ...
                M('order_goods')->where(array('order_id' => $order_id, 'goods_id' => $goods_id))->save($order_goods);
   ...
   public  function recharge(){
	   	...
	   	$Page = new Page($count,10);
	   	$show = $Page->show();
	   	$recharge_list = M('recharge')->where(array('user_id'=>$this->user_id))->order(array('ctime'=>'desc'))->limit($Page->firstRow.','.$Page->listRows)->select();
     ...
     * 查询用户浏览记录
     */
    public function goods_view_history(){
        $goodsLogic = new GoodsLogic();
        $goods_view_list = $goodsLogic->goodsView($this->user_id);
        $this->assign('page',$goods_view_list['show']);// 赋值分页输出
        $this->assign('lists',$goods_view_list['list']);
        $this->assign('active','goods_view_history');
        $this->display();
    }
    ...
    public function del_goods_history(){
        ...
        if($update_info['is_success']){
            $this->success($update_info['message']);
        }else{
            $this->error($update_info['message']);
        }
    }
    ...
    public function show_qr_code(){
        ...
        if($lead_id){
            $ShareLink = urlencode("http_shopv1_host3xx_com_".$lead_id);      //默认分享链接，数据中有效数据为用户id($this->user_id) ,为增加二维码复杂度，添加固定前缀"http_shopv1_host3xx_com_"
            $ShareLink = '/index.php?m=Home&c=Index&a=qr_code&data='.$ShareLink;
            $this->assign('ShareLink',$ShareLink);

        }

        $this->assign('lead_id', $lead_id);
        $this->display();
    }
