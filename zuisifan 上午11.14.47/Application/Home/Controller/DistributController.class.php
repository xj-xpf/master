...
class DistributController extends BaseController {
    ...
    */
    ...
    public function lower_list(){
        ...
        $list = M('users')->field('user_id, email, sex, mobile, head_pic, nickname, level, first_leader, second_leader, reg_time')->where($where)->limit("{$page->firstRow},{$page->listRows}")->order('user_id desc')->select();
    ...
    public function qr_code(){
        ...
        if($this->user['is_distribut'] == 1){
            $ShareLink = urlencode("http_shopv1_host3xx_com_".$this->user_id);      //默认分享链接，数据中有效数据为用户id($this->user_id) ,为增加二维码复杂度，添加固定前缀"http_shopv1_host3xx_com_"
