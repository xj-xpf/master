    ...
    public function app_login($username,$password){
        ...
        if(!$user){
           ...
           $result = array('status'=>-3,'msg'=>'账号异常已被锁定！！！');
            ...
            $ShareLink = urlencode("http_shopv1_host3xx_com_".$user['user_id']); //默认分享链接
            if($user['is_distribut'] == 1)
                $user['share_link'] = ('/index.php?m=Home&c=Index&a=qr_code&data='.$ShareLink);
    ...
    public function reg($username,$password,$password2, $leader = 0){
     ...
     public function get_info($user_id){
        ...
        if(!$user_info)
         ...
         $ShareLink = urlencode("http_shopv1_host3xx_com_".$user_id); //默认分享链接
         if($user_info['is_distribut'] == 1)
             $user_info['share_link'] = ('/index.php?m=Home&c=Index&a=qr_code&data='.$ShareLink);

         $condition['a.uid'] = $user_id;
         $condition['a.use_time'] = 0;
         $condition['b.use_end_time']  = array('gt',time());
         $user_info['coupon_count'] = M('coupon_list')->alias('a')->field('a.id')
                 ->join('ln_coupon  as b on a.cid = b.id ','left')
                 ->where($condition)
                 ->count(); //获取优惠券列表(不包含已过期的优惠券)
//         $user_info['coupon_count'] = M('coupon_list')->where($where)->count(); //获取优惠券列表
...
//        $type = I('get.type',0);
    ...
    public function sms_liuniu_log($mobile,$code,$session_id){
        ...
        if(!$row)
            return array('status'=>-1,'msg'=>'发送失败');
        //发送
        $result = sendMessageRequest($mobile, $code);
        if($result['status'] == 0){
    ...
    public function passwordForApp($user_id, $old_password, $new_password, $is_update=true){
        ...
        $user = $data['result'];
        if(strlen($new_password) < 6)
            return array('status'=>-1,'msg'=>'密码不能少于6位字符','result'=>'');
        /*if($new_password != $confirm_password)
            return array('status'=>-1,'msg'=>'两次密码输入不一致','result'=>'');*/
        //验证原密码
        if($is_update && ($user['password'] != '' && encrypt($old_password) != $user['password']))
