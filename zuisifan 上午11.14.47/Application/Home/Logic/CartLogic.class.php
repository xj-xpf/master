...
class CartLogic extends RelationModel
{
    ...
    function addCart($goods_id,$goods_num,$goods_spec,$session_id,$user_id = 0)
    {
        ...
        if($goods) {
            ...
            if ($catr_count >= 50)
                return array('status' => -9, 'msg' => '购物车最多只能放50种商品', 'result' => '');
            ...
            $where = " goods_id = $goods_id and spec_key = '$spec_key' and status = 1 "; // 查询购物车是否已经存在这商品
            if ($user_id > 0)
            ...
            $shop_price = $spec_price ? $spec_price : $goods['shop_price']; // 如果商品规格没有指定价格则用商品原始价格
            $price = $shop_price;
    ...
    {                   
        ...
        $cartList = M('Cart')->where($where)->order(array('add_time'=>'desc'))->select();  // 获取购物车商品
        //商品总数量     商品总价   与市场价相比优惠金额  默认0
        $anum = $total_price = $cut_fee = $market_cut_fee = 0;

        $order_status = 1;
        //有效商品列表  原购物车列表 去除 失效商品信息
        $cartList_true = array();
        foreach ($cartList as $k=>$val){
            $goods = M('Goods')->where(array('goods_id' => $val['goods_id'], 'is_on_sale'=> 1))->find(); // 找出这个商品
            if($goods){
                if(!empty($cartList[$k]['spec_key'])) // 有商品规格
                {
                    $specGoodsPrice = M('SpecGoodsPrice')
                        ->field("key,key_name,price,store_count")
                        ->where(array('goods_id' => $goods['goods_id'], 'key' => $cartList[$k]['spec_key']))
                        ->find();       // 获取商品对应的规格价钱 库存
                ...
                if($goods['prom_type']&&$goods['prom_id'])
                {
                    ...
                    $data['member_goods_price'] = (($spec_price * 100) * ($user['discount'] * 100)) / 10000;
                ...
                M('Cart')->where(array('id' => $cartList[$k]['id']))->save($data);
                //列表中有效商品
                $val_true = $val;

                $member_goods_price = $data['member_goods_price'] ? $data['member_goods_price'] : $val['member_goods_price'];
                $val_true['goods_price'] = $data['goods_price'];          // 购买价
                $val_true['member_goods_price'] = $member_goods_price;  // 购买价
                $val_true['goods_fee'] = (($val['goods_num'] * $member_goods_price) * 100) / 100;   //单件商品总价
                $val_true['store_count']  = $store_count; // 最多可购买的库存数量

                $anum += $val['goods_num'];     //商品总数量

                $status = $val['goods_num'] <= $store_count ? 1 : 0;

                $val_true['status'] = $status;
                ...
                if($mode == 1){
                    $cut_fee = $cut_fee + ($val['goods_num'] * $spec_price * 100) - ($val['goods_num'] * $member_goods_price * 100);
                }
                $market_cut_fee += ($val['goods_num'] * $goods['market_price'] * 100) - ($val['goods_num'] * $member_goods_price * 100);

                $total_price += $val['goods_num'] * $member_goods_price * 100;
        ...
        if($cartList_true){
            foreach($cartList_true as $key => $val){
                $cartList_true[$key]['original_img'] = M('goods')->where(array('goods_id'=>$val['goods_id']))->getField('original_img');
            }
        }

        $cut_fee = $cut_fee / 100;
        $market_cut_fee = $market_cut_fee / 100;
        $total_price = $total_price / 100;

        $total_price = array('total_fee' =>$total_price , 'cut_fee' => $cut_fee, 'num'=> $anum, 'market_cut_fee' => $market_cut_fee); // 总计
        setcookie('cn',$anum,null,'/');
        if($mode == 1) return array('cartList' => $cartList_true, 'total_price' => $total_price, 'order_status' => $order_status);

        return array('status'=>1,'msg'=>'','result'=>array('cartList' =>$cartList_true, 'total_price' => $total_price, 'order_status' => $order_status));
    }    
...
function cart_freight2($weight , $province, $city, $district)
{
    ...
    public function addOrder($user_id, $address_id, $invoice_title, $coupon_id = 0, $car_price, $user_note)
    {
        ...
        //完全使用余额或积分支付时，支付方式设置
        if($car_price['balance'] || $car_price['pointsFee']){
            $data['pay_name'] = '其他支付方式';
        }
        ...
        if($user['oauth']== 'weixin')
        {
            ...
            push_weixin_message($user['openid'], $wx_content);
        }
        return array('status'=>1,'msg'=>'提交订单成功', 'result'=>$order_id, 'order_amount'=>$data['order_amount']); // 返回新增的订单id
    }
