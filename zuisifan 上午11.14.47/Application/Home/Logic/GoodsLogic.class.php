...
class GoodsLogic extends RelationModel
{
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $priceList = M('goods')->where("goods_id in ($goods_id_str)")->getField('shop_price', true);  //获取价格列表 where("goods_id in($goods_id_str)")->select();
        rsort($priceList);                  //价格倒序排列
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0){
                ...
                $parr[] = array('value' => "{$start}元以上", 'href' => urldecode(U("Goods/$action", $filter_param, '')));;
            } else {
    ...
    function getGoodsIdByAttr($attr)
    {
        ...
        $where = " attr_id in(" . implode(',', $attr_id) . ") and ";

        $count_attr = count($attr_value);   //品牌名称数组
        if($count_attr){
            foreach ($attr_value as $key => $value){
                $where .= " attr_value like '%" .$value . "%' ";        //模糊查询品牌名称
                if($key != ($count_attr-1) ){
                    $where .= " or ";                                   //多个使用 or 链接
                }
            }
        ...
        //$where = " attr_id in(" . implode(',', $attr_id) . ") and attr_value like '%" . implode("','", $attr_value) . "%' ";
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        {
            $item_key_name = array();
            $item_key_name_str = array();
            foreach($v as $k2 => $v2)
            {
                $item_key_name[$v2['item_id']] = $v2['item_id'];
                $item_key_name_str[$v2['item_id']] = $v2['item'];
            }
            ksort($item_key_name);
            ksort($item_key_name_str);
            $item_key = implode('_', array_keys($item_key_name));
            //print_r($keySpecGoodsPrice[$item_key][price]);
            //$item_name = implode(' ', $item_key_name);
            $spec_arr3[$item_key]['item_name'] =  $item_key_name_str;
            $spec_arr3[$item_key]['item'] = $item_key_name;
            $spec_arr3[$item_key]['price'] = $keySpecGoodsPrice[$item_key][price] ? $keySpecGoodsPrice[$item_key][price] : 0; // 价格默认为0
            $spec_arr3[$item_key]['store_count'] =$keySpecGoodsPrice[$item_key][store_count] ? $keySpecGoodsPrice[$item_key][store_count] : 0; //库存默认为0
        }
     ...
     * 获取未选定商品规格
     */
    public function get_spec_other($goods_id, $specArray = null)
    {
        $specIdArray = array();
        //前台未选中规格，返回所有规格
        if($specArray){
            $spec_item_str = implode(',', $specArray);
            foreach($specArray as $value){
                $specItem = M('SpecItem')->where(array('id'=> $value))->getField('spec_id');
                $specIdArray[] = $specItem;
            }
            $spec_id_str = implode(',', $specIdArray);
        }
        ...
        $filter_spec = array();
        if ($keys) {
            //选取规格是，查询选取规格（父类（spec_id））之外的规格信息
            $specImage = M('SpecImage')->where("goods_id = $goods_id and src != '' ")->getField("spec_image_id,src");// 规格对应的 图片表， 例如颜色
            $keys = str_replace('_', ',', $keys);
            $sql = "SELECT a.name,a.order,b.* FROM __PREFIX__spec AS a INNER JOIN __PREFIX__spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys)";
            if($specArray) {
                $sql .= "And b.spec_id NOT IN ($spec_id_str)";
            }
            $sql .= " ORDER BY b.id";
            $filter_spec2 = M()->query($sql);
            foreach ($filter_spec2 as $key => $val) {
        ...
        if($specArray) {
            //选取当前规格（子类（item_id））信息
            $sql2 = "SELECT a.name,a.order,b.* FROM __PREFIX__spec AS a INNER JOIN __PREFIX__spec_item AS b ON a.id = b.spec_id WHERE b.id IN($spec_item_str)  ORDER BY b.id";
            $filter_spec3 = M()->query($sql2);
            foreach ($filter_spec3 as $key => $val) {
                $filter_spec[$val['name']][] = array(
                    'item_id' => $val['id'],
                    'item' => $val['item'],
                    'src' => $specImage[$val['id']],
     ...
     * 获取用户浏览记录
     */
    public function goodsView($user_id, $p=0){

        $where['user_id'] = $user_id;
        $where['status'] = 1;

        $count = M('Goods_history')->where($where)->count();
        if($p != 0){
            //api接口
            $list = M('Goods_history')->where($where)->order(array('add_time'=>'desc'))->page($p, 10)->select();
            foreach($list as $k => $value){
                $goods = M('goods')->field('shop_price')->where(array('goods_id'=>$value['goods_id']))->find();
                $list[$k]['shop_price'] = $goods['shop_price'];
            }
            return array('goods_list'=>$list,'count'=>$count );
        }else{
            //pc端查询
            $page = new Page($count,8);
            $show = $page->show();
            //获取我的收藏列表
            $list = M('Goods_history')->where($where)->order(array('add_time'=>'desc'))->limit($page->firstRow, $page->listRows)->select();
            foreach($list as $k => $value){
                $goods = M('goods')->field('shop_price')->where(array('goods_id'=>$value['goods_id']))->find();
                $list[$k]['shop_price'] = $goods['shop_price'];
            }
     ...
     * 浏览记录 插入
     */
    public function addgoodsView($user_id, $goods_id){

        $is_success = true;     //操作成功失败变量
        $message = '';          //操作提示信息变量
        ...
        $goods_history = M('Goods_history')->where($where)->find();
        //已浏览过商品，更新添加时间
        if($goods_history){
            $goods_history['add_time'] = time();
            $goods_history['count'] = $goods_history['count'] + 1;
            $update_info = M('Goods_history')->where(array('id'=>$goods_history['id']))->save($goods_history);
            ...
            //未浏览过商品，添加
            $goodsInfo = M('Goods')->field("goods_name,shop_price,original_img")->where(array('goods_id'=>$goods_id))->find();
            if($goodsInfo){
                $history_info = array(
                    'goods_id'=>$goods_id,
                    'user_id'=>$user_id,
                ...
                $goods_history_id = M('Goods_history')->add($history_info);
                if($goods_history_id){
                    $message = '添加成功';
                } else {
                    $is_success = false;
        ...
        return array('is_success'=>$is_success, 'message'=>$message);
    }

    /**
     * 删除浏览记录
     */
    public function del_goods_history($goods_history_id){

        $is_success = true;     //操作成功失败变量
        $message = '';          //操作提示信息变量

        $where['id'] = $goods_history_id;
        $data = array(
            'id'     => $goods_history_id,
            'status' => 2,
        );
        $update_info = M('goods_history')->where($where)->save($data);
        if($update_info !== false){
            $message = '删除成功';
        }else{
            $is_success = false;
