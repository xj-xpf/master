...
class CartController extends BaseController
{
    ...
    {
        $ids = I("ids"); // 商品 ids
        $result = M("Cart")->where(array('id'=>array('in',$ids)))->delete(); // 删除id为5的用户数据
        // 查找购物车数量
        $unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
...
//        $user_id = I("user_id",0); // 用户id       
    ...
    public function cart2()
    {
        ...
        $sql = "select c1.name,c1.money,c1.condition, c2.* from __PREFIX__coupon as c1 inner join __PREFIX__coupon_list as c2  on c2.cid = c1.id and c1.type in(0,1,2,3) and order_id = 0 where c2.uid = {$this->user_id} and " . time() . " > c1.use_start_time and " . time() . " < c1.use_end_time and c1.condition <= {$cart_result['total_price']['total_fee']}";
    ...
    public function cart3()
    {
        ...
        if ($_REQUEST['act'] == 'submit_order') {
            ...
            $result = $this->cartLogic->addOrder($this->user_id, $address_id, $invoice_title, $coupon_id, $car_price, $user_note); // 添加订单
            exit(json_encode($result));
        }
