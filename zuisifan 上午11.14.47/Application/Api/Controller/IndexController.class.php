...
class IndexController extends BaseController {
    ...
    public function home(){
        //获取轮播图
        //$data = M('ad')->where('pid = 2')->field(array('ad_link','ad_name','ad_code'))->cache(true,LNSHOP_CACHE_TIME)->select();
...
//                exit($this->http_url);
        ...
        //获取大分类
        //$category_arr = M('goods_category')->where('parent_id=0')->field('id,name')->limit(3)->cache(false,LNSHOP_CACHE_TIME)->select();
        ...
        foreach($category_arr as $c){
            ...
            //$goodsList = M('goods')->where("cat_id in($cat_id)")->limit(4)->cache(false,LNSHOP_CACHE_TIME)->getField("goods_id,goods_name,original_img,shop_price");
            $goodsList = M('goods')->where("cat_id in($cat_id)")->limit(4)->getField("goods_id,goods_name,original_img,shop_price");
    ...
    public function adStart(){
        //获取轮播图
        $data = M('ad')->where('pid = 51317')->order('orderby desc')->select();  
        exit(json_encode(array('status'=>1,'msg'=>'获取成功','result'=>$data)));   
        }  
    /**
     * 获取首页数据
    ...
    public function favourite()
    {
        ...
        //$favourite_goods = M('goods')->field('goods_id,goods_name,shop_price')->where($goods_where)->order('sort DESC')->page($p,10)->cache(true,LNSHOP_CACHE_TIME)->select();
        $favourite_goods = M('goods')->field('goods_id,goods_name,shop_price')->where($goods_where)->order('sort DESC')->page($p,10)->select();
        $json = array(
            'status'=>1,
            'msg'=>'获取成功',
            'result'=>array(
     ...
     * 是否开启app自动更新
     */
    public function getAppAutoupdate(){
        $autoupdate = M('config')->where(array('name'=>'app_autoupdate'))->find();
        if($autoupdate){
            $autoupdate_info = intval($autoupdate['value']);
        }else{
            $autoupdate_info = 0;
        }

        $this->ajaxReturn(array('status'=>1,'msg'=>'获取成功','result'=>$autoupdate_info));
    }
}