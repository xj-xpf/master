...
class GoodsController extends BaseController {
    ...
    public function goodsInfo(){
        ...
        if($keys)
        {
             ...
             $sql  = "SELECT a.name,a.order,b.* FROM __PREFIX__spec AS a INNER JOIN __PREFIX__spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY b.spec_id, b.id";
        ...
        $goods['goods_content'] = '<html><head><meta name="viewport" content="width=device-width,initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no"><style> .content img{display:block;width:100%;height: auto;}</style></head><body class="content">'.htmlspecialchars_decode($goods['goods_content']).'</body></html>';
    ...
    public function goodsFine()
    {
       ...
       switch ($type) {
        ...
        $p = I('p', 1, 'intval');
        $page = new Page($count,10);
        $list = M('goods')->field('is_recommend,is_new,is_hot,goods_id,cat_id,goods_sn,goods_name,shop_price,comment_count')
                          ->where($map)->order("on_time desc")->page($p, 10)->select();
    ...
    {        
        ...
        $list = M('comment')->alias('a')->join(C('DB_PREFIX').'users b on b.user_id = a.user_id','left')
        ...
        if(empty($list)){
            ...
            foreach ($list as $key => $val) {
                //获取商家评论
                $replyList = M('Comment')->where(array('is_show' => 1, 'goods_id' => $goods_id, 'parent_id' => $val['comment_id'] ))->order("add_time desc")->select();
                $list[$key]['reply_list'] = $replyList;
