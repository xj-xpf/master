...
class BaseController extends Controller {
    ...
    public function checkToken(){
        ...
        $check_arr = array(
            ...
            'user'=>array('userinfo','updateuserinfo','password','getaddresslist','addaddress','del_address','setdefaultaddress','getcouponlist','getgoodscollect','getorderlist','getorderdetail','cancelorder','orderconfirm','add_comment','account','return_goods_list','return_goods_info','return_goods_status','return_goods','editavator','mobile_validate','withdrawals','usermoneyrecord','userrecharge'), // 需要验证登录的方法 
            'cart'=>array('cart2','cart3'), // 需要验证登录的方法 
            'goods'=>array('collectgoods','goodsview','addgoodsview','del_goods_history'), // 需要验证登录的方法
            'distribut'=>array('index', 'lower_list', 'order_list', 'qr_code'),
            );
        if($this->token){
            $check_arr['cart'][] = 'cartlist';
            $check_arr['cart'][] = 'addcart';
            $check_arr['cart'][] = 'batchaddcart';
        }
