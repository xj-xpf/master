...
{    
    ...
    public function addUser($user)
    {
		...
		if($user['email'] && $user['mobile']){
			$where = " email='".$user['email']."'or mobile='".$user['mobile']."'";
		}else{
			if($user['email']){
    	...
    	if(M('users')->where($where)->count()>0){
    	...
    	if(!$user_id){
    		...
    		if($pay_points > 0){
				$account_log_info = accountLog($user_id, 0 , $pay_points , '会员注册赠送积分'); // 记录日志流水
				if(!$account_log_info['is_success']){
					array('status'=>-2,'msg'=>'添加失败');
				}
			}
    		// 会员注册送优惠券
