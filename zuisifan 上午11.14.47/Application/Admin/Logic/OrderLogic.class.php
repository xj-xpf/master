...
class OrderLogic extends RelationModel
{
    ...
    public function getOrderGoods($order_id){
        ...
        $spec_image = M('spec_image');
		foreach($res as $key => $value){

			$key_array = explode('_', $value['spec_key']);
			$key_array_str = implode(',', $key_array);
			$spec_image_info = $spec_image
				->where(array('goods_id'=>$value['goods_id'],
								'spec_image_id'=>array('in', $key_array_str),
								'url'=>array('not', 'null')
								))->find();

			if($spec_image_info){
				$res[$key]['spec_image_src']=$spec_image_info['src'];
			}
		}
    ...
    public function orderProcessHandle($order_id,$act){
    	...
    	switch ($act){
    		...
    		case 'cancel': //取消确认
    			...
    			break;
    		case 'invalid': //作废订单
				//退还付款、余额、积分  其中付款和余额 都退还至余额
				$this->invalid_order($order_id);
	...
	//作废订单，退还金额 余额 积分
	function invalid_order($order_id){
		$order_info = M('order')->where(array('order_id'=>$order_id))->find();
		if($order_info){
			//用户未取消订单
			if($order_info['order_status'] != 3){
				//订单部分支付（余额，积分） order_status = 0 ,pay_status = 0
				if($order_info['pay_status'] == 0){
					$return_money = $order_info['user_money'];
				}
				//订单全部支付 （余额，积分，现金）order_status = 0 ,pay_status = 0
				if($order_info['pay_status'] == 1){
					$return_money = $order_info['user_money'] + $order_info['order_amount'];
				}
				accountLog($order_info['user_id'], $return_money, $order_info['integral'], '订单作废',0);
			}
