...
class OrderController extends BaseController {
    ...
    */
    ...
    public function pay_cancel($order_id){
    	if(I('remark')){
    		...
    		if($data['refundType'] == 0 && $data['amount']>0){
                $account_log_info = accountLog($data['user_id'], $data['amount'], 0,  '退款到用户余额');
    ...
    public function return_info()
    {
        ...
        //获取订单商品信息
        $where_order_goods['goods_id'] = $return_goods['goods_id'];
        $where_order_goods['order_id'] = $return_goods['order_id'];
        if($return_goods['spec_key']){
            $where_order_goods['spec_key'] = $return_goods['spec_key'];
        }

        $user = M('users')->where(array('user_id' => $return_goods['user_id']))->find();
        $goods = M('goods')->where(array('goods_id' => $return_goods['goods_id']))->find();
   ...
   {                
    ...
    public function order_action(){    	
        ...
        if($action && $order_id){
            ...
            if($action != 'remove'){
                $res = $orderLogic->orderActionLog($order_id,$action,I('note'));        //删除操作不添加操作日志；
            }else{
                $res = true;
            }
