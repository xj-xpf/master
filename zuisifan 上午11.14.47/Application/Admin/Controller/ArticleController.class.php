...
class ArticleController extends BaseController {
    ...
    private $article_system_id = array(1, 2, 3, 4, 5, 6);//系统默认的文章分类id，不可删除
    private $article_main_system_id = array(2);//系统保留分类，不允许在该分类添加文章
    private $article_top_system_id = array(1);//系统分类，不允许在该分类添加新的分类
    private $article_able_id = array(1, 2, 3, 4);//系统预定义文章id，不能删除。此文章用于商品详情售后服务
    ...
    public function articleList(){
        ...
        if($res){
        	foreach ($res as $val){
        		...
        		$val['add_time'] = date('Y-m-d H:i:s', $val['add_time']);           //添加时间
                $val['publish_time'] = date('Y-m-d H:i:s', $val['publish_time']);   //发布时间
        		$list[] = $val;
        	}
        }
